...
pub fn projectile_system(
    ...
    for (entity, mut projectile, mut transform, Speed(speed), MoveDirection(direction)) in
        &mut query
    {
    ...
    for (projectile_entity, projectile_transform) in &projectiles_query {
        for (agent_entity, mut agent_transform, collider, mut lives) in &mut agents_query {
            ...
            if has_hit {
                ...
                let new_translation =
                    (agent_transform.translation - projectile_transform.translation).normalize()
                        * 0.2;
